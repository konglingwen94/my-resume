...
在公司任职期间，我独立开发完成了"CSAP 剑桥暑期学生课程"小程序，"九鼎锐创"网站，"930 家庭医生"公众号，"股票众筹"网站对应的 4 个**管理后台项目**。我主要的工作是根据 UI 设计稿完成页面的样式布局，数据渲染等内容。所有项目均采用前后端分离的开发模式，我负责和服务端开发人员联合调试数据接口，并完成对各模块数据的增删改查功能。在项目开发期间配合公司测试人员完成应用`bug`的循环修复工作。在优化完善项目的过程中，针**成功请求服务端接口后同步新数据到应用页面**的功能时，我利用 Vue 模板绑定响应化数据后自动更新页面的特点，采取直接更新组件初始化状态的方法，完成了减少重复接口的请求次数，加快了页面重新渲染的速度、优化浏览器性能、提升用户体验等优化内容。
